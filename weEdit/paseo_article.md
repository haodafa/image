...
Paseo 还有个更重的 Orchestrator 模式——这不是简单的"让 Agent 聊天"，而是一套完整的 11 阶段工程流程：从调研、规划、实现到审计、清理、QA，每个阶段都有专门角色的 Agent 负责，实现和审计严格分离（写代码的不做审查，审查的不改代码）。它甚至有心跳机制，每 5 分钟自检一次进度，卡死了自动重启。设计很认真，但标记着 Unstable，实际跑起来偶尔会出现 Agent 之间协调不畅的情况。这个方向值得关注，但目前还不建议用于正经项目。
...
**数据全在本地。** `~/.paseo/` 下一堆 JSON 文件，人类可读，想看就看。没有遥测，没有强制登录。远程连接走端到端加密（Curve25519 密钥交换 + XSalsa20-Poly1305 加密），中继服务器是零知识设计。
